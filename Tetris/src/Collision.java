...
public class Collision {
	...
	public static boolean collideWithBlock(Block b, int direction) {
		...
		switch (direction) {																			//Eine Kollision ist nur möglich nach unten oder oben 
		case -1:																						//Eine Berührung mit den Wänden ist egal  
			if (b.getY() > 0) {
				if (b.getX() > 0) {
	...
	public static void checkFullRow(int multiplier) {				//Bei einer kompletten Reihe wird der Punktestand übertragen und die letzte Reihe wird gelöscht 
	...
	private static void delRow(int row, int multiplier) {		//Wenn eine Reihe komplett ist, wird sie gelöscht und die restlichen Blöke rutschen nach  
	...
	private static void checkLoose() {																//Der Spielstand wird auf GameOver gesetzt
		for (int x = 0; x < Game.map.length; x++) {
